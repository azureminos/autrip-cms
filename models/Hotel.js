...
Hotel.schema.methods.updateCity = function (callback) {
	...
	if (hotel.city) {
		...
		keystone
			.list('City')
			.model.findById(hotel.city.toString())
			.exec(function (err, item) {
				if (err || !item) return callback();
		...
		.exec(function (err, items) {
			...
			_.each(items, function (item) {
				promises.push(function (callback) {
					console.log(
						`>>>>Hotel.cleanupRooms, Checking room[${item.name}] against [${hotel.name}, ${hotel._id}]`,
						hotel.rooms
					);
					...
					if (!hotel.rooms || !isFound) {
						console.log(
							`>>>>Hotel.cleanupRooms, Removing [${hotel.name}] from room[${item.name}].hotel`,
							item.hotel
						);
						keystone
							.list('HotelRoom')
...
Hotel.schema.methods.updateRooms = function (callback) {
	...
	// Get all rooms
	console.log(
		`>>>>Hotel.updateRooms, Looping through hotel[${hotel.name}].rooms`,
		hotel.rooms
	);
		...
		promises.push(function (callback) {
			keystone
				...
				.exec(function (err, item) {
					...
					console.log(
						`>>>>Hotel.updateRooms, Checking if room[${item.name}].hotel is [${
							hotel.name
						}, ${hotel._id.toString()}]`,
						item.hotel
					);
					if (
						!item.country
						|| country._id.toString() != item.country.toString()
					) {
						item.hotel = hotel._id;
						console.log(
							`>>>>Hotel.updateRooms, Set [${
								hotel.name
							}, ${hotel._id.toString()}] as room[${item.name}].hotel`,
							item.hotel
						);
						keystone
							.list('HotelRoom')
