...
PackageHotel.schema.methods.cleanupPackage = function (callback) {
	...
	keystone
		.list('TravelPackage')
		.model.findOne({ packageHotels: packageHotel._id })
		.exec(function (err, item) {
			if (err || !item) return callback();
			if (
				!packageHotel.package
				|| (packageHotel.package
					&& item._id.toString() != packageHotel.package.toString())
			) {
...
PackageHotel.schema.methods.updatePackage = function (callback) {
	...
	if (packageHotel.package) {
		...
		keystone
			.list('TravelPackage')
			.model.findById(packageHotel.package.toString())
			.exec(function (err, item) {
				if (err || !item) return callback();
		...
		[
			function (callback) {
				if (packageHotel.isModified('package')) {
					console.log(
						'>>>>packageHotel.package changed, calling packageHotel.cleanupPackage'
					);
			...
			function (callback) {
				if (packageHotel.isModified('package')) {
					console.log(
						'>>>>packageHotel.package changed, calling packageHotel.updatePackage'
					);
