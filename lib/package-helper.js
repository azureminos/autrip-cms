...
exports.enhanceInstance = (instPackage, userId) => {
	console.log('>>>>PackageHelper.enhanceInstance', { instPackage, userId });
	const isCustomised = instPackage.isCustomised || false;
	const isCustomisable = instPackage.isCustomisable || false;
	const statusInstance = instPackage.status;
	const member
	...
	if (!isCustomised) {
		...
		if (
			extras.statusInstance === Instance.status.INITIATED
			|| extras.statusInstance === Instance.status.SELECT_ATTRACTION
		) {
			extras.step = 0;
		} else if (extras.statusInstance === Instance.status.SELECT_HOTEL) {
			extras.step = 1;
		} else if (extras.statusInstance === Instance.status.REVIEW_ITINERARY) {
			extras.step = 2;
		} else if (extras.statusInstance === Instance.status.PENDING_PAYMENT) {
			extras.step = 3;
		}
		...
		if (
			extras.statusInstance === Instance.status.INITIATED
			|| extras.statusInstance === Instance.status.SELECT_ATTRACTION
			|| extras.statusInstance === Instance.status.SELECT_HOTEL
			|| extras.statusInstance === Instance.status.REVIEW_ITINERARY
		) {
			extras.step = 0;
		} else if (extras.statusInstance === Instance.status.PENDING_PAYMENT) {
			extras.step = 1;
