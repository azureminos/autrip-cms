...
exports.getPreviousStatus = (isCustomised, status) => {
	...
	if (!isCustomised) {
		if (status === Instance.status.SUBMIT_PAYMENT) {
			return Instance.status.INITIATED;
		}
	} else {
		if (status === Instance.status.SELECT_HOTEL) {
			return Instance.status.SELECT_ATTRACTION;
		} else if (status === Instance.status.SUBMIT_PAYMENT) {
...
exports.getNextStatus = (isCustomised, status) => {
	...
	if (!isCustomised) {
		if (status === Instance.status.INITIATED) {
			return Instance.status.SUBMIT_PAYMENT;
		}
	} else {
		if (
